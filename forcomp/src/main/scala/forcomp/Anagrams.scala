...
object Anagrams {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val k = numbers.foldLeft(3)((m: Int, n: Int) => m + n)
    print(k)
  }
  ...
  /** `Occurrences` is a `List` of pairs of characters and/home/slava/scala/FirstPart positive integers saying
  ...
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(c => c)
    .map({case (c, s) => (c, s.length)}).toList.sorted
